...
DEFAULT_SUMMARY_ROOT = REPO_ROOT / "experiments/stage136_render_aware_scale_sweep_smoke"
...
def summarize(rows):
    ...
    for row in rows:
        grouped[(row["predictor_name"], row["setting_label"], float(row["adapter_delta_scale"]))].append(row)
    out = []
    for (predictor, setting, scale), items in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1], item[0][2])):
        first = items[0]
        out.append({
...
def best_summary_row(summary_rows):
    return max(
        summary_rows,
        key=lambda row: (
...
def write_report(summary, summary_rows, path):
    ...
    lines = [
        ...
        f"- mean selected PSNR: `{format_float(best['mean_selected_predicted_psnr'])}`",
        f"- mean delta vs base: `{format_float(best['mean_delta_psnr_vs_base'])}`",
        "",
        "## Summary",
        "",
...
def main():
    ...
    if protocol["protocol_name"] != "render_aware_adapter_delta_scale_calibration_v1":
        raise ValueError("Stage136 expects the Stage135 render-aware adapter-delta scale protocol")
    if policy["policy_name"] != "compressed_deterministic_value_only_residual_codec_v1":
        raise ValueError("Stage136 expects the Stage123 compressed deterministic codec policy")
    ...
    best = best_summary_row(summary_rows)
    rows_csv = args.summary_root / "stage136_render_aware_scale_sweep_smoke_rows.csv"
    summary_csv = args.summary_root / "stage136_render_aware_scale_sweep_smoke_summary.csv"
    summary_json = args.summary_root / "stage136_render_aware_scale_sweep_smoke_summary.json"
    report_md = args.summary_root / "stage136_render_aware_scale_sweep_smoke_report.md"
    write_csv(rows, rows_csv, ROW_FIELDS)
    write_csv(summary_rows, summary_csv, SUMMARY_FIELDS)
    summary = {
        "stage": 136,
        "mode": "render-aware adapter-delta scale sweep smoke",
    ...
    print(json.dumps({"summary": str(summary_json), "row_count": len(rows), "best_summary_row": best}, indent=2))

...
DEFAULT_STAGE35_CSV = REPO_ROOT / "experiments/stage35_dense_anchor_attribute_oracle_selector_rd/stage35_dense_anchor_attribute_oracle_selector_rd.csv"
DEFAULT_HEAVY_ROOT = Path("/mnt/hdd2tC/tmp/opencode/mono_dfcgs_runs/stage36_dense_oracle_actual_bitstream_rd")
DEFAULT_SUMMARY_ROOT = REPO_ROOT / "experiments/stage36_dense_oracle_actual_bitstream_rd"
...
def write_csv(rows, path):
    fields = [
        "sample", "method", "reference_gap", "total_frames", "keyframe_count", "indices",
        "raw_path", "zlib_path", "raw_bitstream_bytes", "zlib_bitstream_bytes", "raw_mib_per_frame",
        "zlib_mib_per_frame", "zlib_savings_percent_vs_raw_bitstream", "linear_all_psnr", "adapter_all_psnr",
        "delta_all_psnr", "linear_middle_psnr", "adapter_middle_psnr", "delta_middle_psnr", "linear_all_ssim",
        "adapter_all_ssim", "linear_middle_ssim", "adapter_middle_ssim", "max_roundtrip_abs_diff", "mean_roundtrip_mse",
    ]
...
def selector_comparison(rows, rate_key):
    ...
    for (sample, gap), methods in sorted(grouped.items()):
        if "uniform" not in methods or "dense_anchor_attr_oracle" not in methods:
            continue
        uniform = methods["uniform"]
        oracle = methods["dense_anchor_attr_oracle"]
        out.append({
            "sample": sample,
            "reference_gap": gap,
            "rate_kind": rate_key,
            "uniform_rate_mib_per_frame": uniform[rate_key],
            "oracle_rate_mib_per_frame": oracle[rate_key],
            "rate_delta_mib_per_frame": oracle[rate_key] - uniform[rate_key],
...
def write_comparison_csv(rows, path):
    fields = [
        "sample", "reference_gap", "rate_kind", "uniform_rate_mib_per_frame", "oracle_rate_mib_per_frame",
...
def plot_rd(rows, rate_key, metric_key, ylabel, title, out_path):
    ...
    colors = {"uniform": "#1f77b4", "dense_anchor_attr_oracle": "#9467bd"}
    markers = {"uniform": "o", "dense_anchor_attr_oracle": "^"}
...
def parse_args():
    ...
    parser.add_argument("--stage35_csv", type=Path, default=DEFAULT_STAGE35_CSV)
...
def main():
    ...
    source_rows = load_stage35(args.stage35_csv)
    anchor_maps = {}
    ...
    for row in source_rows:
        ...
        rows.append({
            **{k: row[k] for k in [
                "sample", "method", "reference_gap", "total_frames", "keyframe_count", "linear_all_psnr",
                "adapter_all_psnr", "delta_all_psnr", "linear_middle_psnr", "adapter_middle_psnr",
    ...
    csv_path = args.summary_root / "stage36_dense_oracle_actual_bitstream_rd.csv"
    raw_comparison_csv = args.summary_root / "stage36_raw_selector_comparison.csv"
    zlib_comparison_csv = args.summary_root / "stage36_zlib_selector_comparison.csv"
    summary_path = args.summary_root / "stage36_dense_oracle_actual_bitstream_rd_summary.json"
    ...
    for rate_key, rate_name in [("raw_mib_per_frame", "raw"), ("zlib_mib_per_frame", "zlib")]:
        for metric_key, ylabel, filename in [
            ("adapter_all_psnr", "Adapter all-frame PSNR (dB)", f"stage36_{rate_name}_adapter_all_psnr_rd.png"),
            ("adapter_middle_psnr", "Adapter middle-only PSNR (dB)", f"stage36_{rate_name}_adapter_middle_psnr_rd.png"),
        ]:
            out_path = args.summary_root / filename
            plot_rd(rows, rate_key, metric_key, ylabel, f"Stage 36 Dense Oracle {rate_name.upper()} Bitstream RD", out_path)
            plots.append(str(out_path))

    all_values = [row["selector_delta_adapter_all_psnr"] for row in raw_comparisons]
    middle_values = [row["selector_delta_adapter_middle_psnr"] for row in raw_comparisons]
    summary = {
        "stage": 36,
        "mode": "dense oracle actual q8 anchor bitstream RD",
        "stage33_manifest": str(args.stage33_manifest),
        "stage35_csv": str(args.stage35_csv),
        ...
        "notes": "Quality comes from Stage35; rate is measured from actual Stage36 raw/zlib q8 anchor bitstreams.",
    }

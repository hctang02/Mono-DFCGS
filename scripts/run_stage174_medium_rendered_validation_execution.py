...
DEFAULT_STAGE173_SCHEDULE_ROWS = REPO_ROOT / "experiments/stage173_medium_rendered_validation_protocol/stage173_medium_validation_schedule_rows.csv"
...
DEFAULT_SUMMARY_ROOT = REPO_ROOT / "experiments/stage174_medium_rendered_validation_execution"
DEFAULT_HEAVY_ROOT = Path("/data/hctang/tmp/opencode/mono_dfcgs_runs/stage174_medium_rendered_validation_execution")
...
def summarize(rows):
    ...
    for (category, schedule), group in sorted(groups.items()):
        ...
        out.append({
            ...
            "reused_count": sum(1 for row in group if str(row["row_source"]).startswith("stage16") or row["row_source"] == "stage170"),
            "new_render_count": sum(1 for row in group if row["row_source"] == "stage174_rendered"),
...
def save_contact_sheet(rows, image_cache, heavy_root):
    ...
    rendered_new = [row for row in rows if row["row_source"] == "stage174_rendered"]
    if not rendered_new:
        return None
    cells = []
    ...
    path = heavy_root / "stage174_medium_rendered_validation_contact_sheet.jpg"
...
def parse_args():
    ...
    parser.add_argument("--stage173_schedule_rows", type=Path, default=DEFAULT_STAGE173_SCHEDULE_ROWS)
    ...
    parser.add_argument("--max_new_renders", type=int, default=80)
...
def main():
    ...
    targets = target_map(read_csv(args.stage173_targets))
    protocol_rows = read_csv(args.stage173_schedule_rows)
    ...
    for protocol in protocol_rows:
        ...
        if protocol["expected_status"] == "target_keyframe_no_middle_render" or task is None:
            rows.append(add_context(empty_metric_row(target, protocol["schedule"], left, right, "target is transmitted keyframe under this schedule"), target, protocol["category"], "stage174_keyframe_marker"))
            continue
        ...
        rows.append(add_context(rendered_metric_row(target, protocol["schedule"], task, selected_half, selected_metrics, original_metrics, payload_bytes), target, protocol["category"], "stage174_rendered"))
        new_renders += 1
    ...
    rows_csv = args.summary_root / "stage174_medium_validation_rows.csv"
    summary_csv = args.summary_root / "stage174_medium_validation_summary.csv"
    source_csv = args.summary_root / "stage174_medium_validation_source_summary.csv"
    package_json = args.summary_root / "stage174_medium_rendered_validation_execution_package.json"
    report_md = args.summary_root / "stage174_medium_rendered_validation_execution_report.md"
    ...
    expected_new_renders = sum(int(row["requires_stage174_render"]) for row in protocol_rows)
    decision = "medium_validation_ready_for_decision" if protocol_complete and new_renders == expected_new_renders else "medium_validation_incomplete"
    package = {
        "stage": 174,
        "status": "medium_rendered_validation_execution_packaged",
        "decision": decision,
        ...
        "reused_row_count": sum(1 for row in rows if row["row_source"] in ("stage167", "stage168", "stage170")),
    ...
    print(json.dumps({"package": str(package_json), "decision": decision, "new_render_count": new_renders, "contact_sheet": contact_sheet}, indent=2))

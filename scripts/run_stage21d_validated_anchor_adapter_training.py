...
DEFAULT_HEAVY_ROOT = Path("/mnt/hdd2tC/tmp/opencode/mono_dfcgs_runs/stage21d_validated_anchor_adapter_training")
DEFAULT_SUMMARY_ROOT = REPO_ROOT / "experiments/stage21d_validated_anchor_adapter_training"
...
def main():
    ...
    best_step = 0
    best_checkpoint = save_model(model, args.heavy_root / "stage21d_best_anchor_adapter.safetensors")
    ...
    final_checkpoint = save_model(model, args.heavy_root / "stage21d_final_anchor_adapter.safetensors")

    train_csv = args.summary_root / "stage21d_train_rgb_losses.csv"
    validation_csv = args.summary_root / "stage21d_validation_log.csv"
    initial_eval_csv = args.summary_root / "stage21d_initial_eval.csv"
    final_eval_csv = args.summary_root / "stage21d_final_eval.csv"
    best_eval_csv = args.summary_root / "stage21d_best_eval.csv"
    gap_eval_csv = args.summary_root / "stage21d_best_gap_eval_summary.csv"
    ...
    summary = {
        "stage": "21d",
        "mode": "validated residual-zero Gaussian-anchor-only adapter training",
        ...
        "notes": "Validation selects the adapter checkpoint with the largest robot eval margin over q8 linear-anchor rendering. Inputs remain q8 keyframe anchors plus timestamp only.",
    }
    summary_path = args.summary_root / "stage21d_validated_anchor_adapter_training_summary.json"

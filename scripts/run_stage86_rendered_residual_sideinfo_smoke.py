...
DEFAULT_SUMMARY_ROOT = REPO_ROOT / "experiments/stage86_rendered_residual_sideinfo_smoke"
...
def apply_topk_residual(base_attrs, target_attrs, keep_fraction):
    ...
    masked[:, keep_idx, :] = residual[:, keep_idx, :]
    return unflatten_static_anchor(base_attrs + masked), keep_count


def render_psnr(anchor, target, background, opt):
    pred = render_anchor(anchor, background, opt).clamp(0.0, 1.0)
...
def write_report(summary, summary_rows, path):
    lines = [
        ...
        "## Summary",
        "",
        "| base | codec | gap | keep | side MiB/frame | base PSNR | side PSNR | delta | positives |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary_rows:
        lines.append(
            f"| {row['base_method']} | {row['codec']} | {row['reference_gap']} | {row['keep_fraction']} | {row['mean_side_info_mib_per_intermediate_frame']} | {row['mean_base_psnr']} | {row['mean_sideinfo_psnr']} | {row['mean_delta_psnr_vs_base']} | {row['positive_delta_count']} |"
        )
    lines.extend([
        "",
        "## Notes",
        "",
        "- This is an optimistic rendered smoke: residual values are not quantized yet.",
        "- Side-info rate is estimated from q8 residual attrs plus Gaussian indices and must be counted in total rate if transmitted.",
        "- Dense target anchors are used only to construct encoder-side residual side-info for this smoke.",
    ])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def parse_args():
    ...
    parser.add_argument("--keep_fractions", nargs="+", type=float, default=[0.0, 0.1, 0.25])
    parser.add_argument("--side_bits", type=int, default=8)
...
def main():
        ...
        for task in tasks:
            ...
            for method, base_attrs in base_attrs_by_method.items():
                ...
                for keep_fraction in args.keep_fractions:
                    side_anchor, keep_count = apply_topk_residual(base_attrs, target_attrs, keep_fraction)
                    ...
                    rows.append({
                        ...
                        "side_info_mib_per_intermediate_frame": side_info_mib(int(base_attrs.shape[1]), keep_count, int(base_attrs.shape[-1]), args.side_bits),
                        "base_psnr": base_psnr,
    ...
    rows_csv = args.summary_root / "stage86_rendered_residual_sideinfo_rows.csv"
    summary_csv = args.summary_root / "stage86_rendered_residual_sideinfo_summary.csv"
    summary_json = args.summary_root / "stage86_rendered_residual_sideinfo_smoke_summary.json"
    report_md = args.summary_root / "stage86_rendered_residual_sideinfo_smoke_report.md"
    write_csv(rows, rows_csv, ROW_FIELDS)
    write_csv(summary_rows, summary_csv, SUMMARY_FIELDS)
    summary = {
        "stage": 86,
        "mode": "rendered residual side-info smoke",
        ...
        "notes": [
            "Optimistic rendered smoke only; residual values are not quantized.",
            "Side-info rate must be counted if residuals are transmitted.",
            "Dense target anchors are used to construct encoder-side residual side-info for this smoke.",
        ],
    }

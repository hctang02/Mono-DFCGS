...
## 2026-06-28：Stage98 Residual Importance Predictor Smoke

### 目标

验证一个 feed-forward residual importance predictor 是否能从 decoder 可得特征预测 teacher top10 residual-energy Gaussian mask，为后续 deployable residual side-info generation 做 baseline。

### 操作计划

- 新增 `scripts/run_stage98_residual_importance_predictor_smoke.py`。
- 输入 Stage97 residual predictor task manifest。
- 训练小型 MLP：输入 left/right keyframe anchor attrs、base anchor attrs、endpoint diff、normalized time、base method id；输出 Gaussian importance logit。
- 标签：training/encoder-side teacher top10 residual-energy mask，来自 target dense anchor。
- 训练 smoke 使用小样本 train tasks，评估小样本 eval tasks，避免大规模训练。
- 对比 endpoint-difference baseline。
- 输出 top10 precision、total residual energy recall、relative recall vs oracle top10。
- 不保存 model checkpoint 或 heavy tensors。
- 运行前按要求检查 `nvidia-smi`，并选择空闲 GPU。
...
| candidate | base | gap | precision@keep | energy recall | oracle recall | relative recall |
|---|---|---:|---:|---:|---:|---:|
| endpoint_diff_baseline | linear | 4 | 0.3122626145680745 | 0.2655188664793968 | 0.6100062330563863 | 0.42121463517347973 |
| endpoint_diff_baseline | linear | 8 | 0.24724181989828745 | 0.23293344179789224 | 0.6551332771778107 | 0.34736502170562744 |
| endpoint_diff_baseline | linear | 16 | 0.2551998645067215 | 0.29756257434686023 | 0.6923819482326508 | 0.38490988314151764 |
| mlp_importance | linear | 4 | 0.310047022998333 | 0.2947804356614749 | 0.6100062330563863 | 0.46986613670984906 |
| mlp_importance | linear | 8 | 0.292729248603185 | 0.2800879975159963 | 0.6551332771778107 | 0.4127636154492696 |
| mlp_importance | linear | 16 | 0.26433352132638294 | 0.3037059058745702 | 0.6923819482326508 | 0.4129539728164673 |
| endpoint_diff_baseline | stage65_adapter | 4 | 0.27450714260339737 | 0.12835493932167688 | 0.21536186089118323 | 0.6087801853815714 |
| endpoint_diff_baseline | stage65_adapter | 8 | 0.2631578892469406 | 0.13744011024634042 | 0.27263158559799194 | 0.5076933900515238 |
| endpoint_diff_baseline | stage65_adapter | 16 | 0.18972689906756082 | 0.11895131816466649 | 0.2499206562836965 | 0.5244861443837484 |
| mlp_importance | stage65_adapter | 4 | 0.3889039605855942 | 0.14692467202742895 | 0.21536186089118323 | 0.6976491312185923 |
| mlp_importance | stage65_adapter | 8 | 0.37384698788324994 | 0.1679172863562902 | 0.27263158559799194 | 0.6167584160963694 |
| mlp_importance | stage65_adapter | 16 | 0.28431904315948486 | 0.14672287801901499 | 0.2499206562836965 | 0.6156713565190634 |
...
## 2026-06-29：Stage118 Compressed Deterministic Value-Only Codec Smoke

### 目标

Implement a lossless compressed deterministic-index value-only residual side-info codec. The decoder still reproduces selected indices deterministically; the bitstream transmits compressed metadata and q residual values only.

### 操作计划

- Add compressed deterministic encode/decode helpers in `mono_dfcgs/residual_sideinfo_codec.py`.
- Add a Stage118 smoke script based on Stage115 to compare fixed index+value, raw deterministic value-only, and compressed deterministic value-only payloads.
- Verify compressed deterministic decode matches raw deterministic decode with `max_abs_diff = 0.0`.
- Compare compressed deterministic q6/top10 bytes against Stage96 entropy-coded index+value reference rows.
- Do not train, render, or save anchors/checkpoints/heavy tensors.
...
- Compressed deterministic value-only side-info decodes identically to raw deterministic and fixed index+value payloads under the same selected indices.
...
## 2026-06-29：Stage121 Broader Rendered Compressed Deterministic Validation

### 目标

Validate the Stage120 rendered shortlist on a broader 60-task eval slice.

### 操作计划

- Add a Stage121 broader rendered validation script using the Stage120 rendering/accounting logic.
- Evaluate q6/top10, q5/top10, q4/top10, and q4/top20; drop q6/top5.
- Use `max_tasks=60` eval tasks by default.
- Record PSNR, delta vs base, delta vs q6/top10, side-info bytes, direct total rate, and amortized total rate.
- Keep dense target anchors encoder-side only for residual values.
- Do not train or save anchors/checkpoints/heavy tensors.
- Check `nvidia-smi` before Python execution.
...
运行前检查 `nvidia-smi`：GPU0 忙、GPU1 有进程、GPU4 有进程，GPU2/3/5/6/7 空闲，因此使用 `CUDA_VISIBLE_DEVICES=2`。
...
- q4/top20 remains the best candidate on the broader 60-task validation: higher PSNR than q6/top10 and slightly lower direct/amortized rate.

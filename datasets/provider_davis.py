...
class DAVISDataset(Dataset):
    def __init__(self, opt, training=True, shuffle=False, nearby_range=4):
        self.opt = opt
        self.training = training
        self.shuffle = shuffle
        if nearby_range < opt.output_frames - 1:
            print(f"Warning: nearby_range is less than output_frames - 1 on davis, nearby_range: {nearby_range}, output_frames: {opt.output_frames}, change nearby_range to {opt.output_frames - 1}")
        ...
        self.mask_root = osp.join(opt.root_path, "Annotations_unsupervised", "Full-Resolution")
        ...
        for set_idx, set_name in enumerate(self.set_names):
            ...
            image_files = sorted(glob.glob(osp.join(set_dir, "*.jpg")))
            if not image_files:
    ...
    def __getitem__(self, index):
        ...
        if self.opt.output_frames > 1 and seq_length > 1:
            ...
            for offset in offsets:
                ...
                pair_depth_image = self.transform(pair_depth_image)
